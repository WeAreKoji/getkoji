...
const logStep = (step: string, details?: any) => {
  ...
  console.log(`[GET-PAYOUT-INFO] ${step}${detailsStr}`);
};
...
serve(async (req) => {
  ...
  try {
    ...
    // Get payout schedule from account
    ...
    return new Response(JSON.stringify({
      ...
      dashboardUrl: `https://dashboard.stripe.com${isTestMode ? '/test' : ''}/connect/accounts/${creatorProfile.stripe_account_id}`,
    }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 200,
    });
